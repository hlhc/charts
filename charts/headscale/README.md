...
| `headscale.pdb.unhealthyPodEvictionPolicy`                        | Policy for evicting unhealthy pods. `AlwaysAllow` allows draining nodes even when the pod is not ready, preventing stuck drains. Requires Kubernetes >= 1.26.                                                                                   | `AlwaysAllow`                                            |
...
| `headplane.image.tag`                                       | headplane image tag                                                                                                                         | `0.6.2`                         |
...
| `headplane.oidc.clientSecret`                               | OIDC client secret (stored in a managed Secret). Ignored if existingSecret is set.                                                          | `""`                            |
| `headplane.oidc.existingSecret`                             | Name of an existing secret containing the OIDC client secret                                                                                | `""`                            |
| `headplane.oidc.existingSecretKey`                          | Key in the existing secret containing the OIDC client secret                                                                                | `client_secret`                 |
...
| `headplane.apiKey.autoGenerate`                             | Automatically generate a headscale API key via a post-install/post-upgrade Job.                                                             | `false`                         |
